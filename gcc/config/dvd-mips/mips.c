...
/*
** modifications for LXR4080 follows.
**
     ...
     enum block_move_type move_type;
{
  ...
  while (bytes > 0)
      ...
      else if (bytes >= 4 && align >= 4)
	{
      ...
      else if (bytes >= 4 && ! TARGET_MIPS16 && ! TARGET_NO_UNALIGN )
	{
	  if (BYTES_BIG_ENDIAN)
	    {
...
#define MIPS_ISA_DEFAULT 1
  ...
  /*
  **  -lxr=ce/none setting default
  */
  if (lxr_config_string == (char *)0)
  {
    lxr_config_string="normal";
  }

  /*
  **  check -mlxr=XXX
  */
  if (!strcmp(lxr_config_string, "ce"))
  {
    lxr_config = LXR_CE;
  } else if (!strcmp(lxr_config_string, "normal"))
  {
    lxr_config = LXR_NORMAL;
  } else
  {
    fatal( "illegal lxr= configuration (ce/normal)\n");
  }
