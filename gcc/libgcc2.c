...
#if defined(CROSS_COMPILE) && !defined(inhibit_libc) && !defined(__CYGWIN32__) && !defined(__MINGW32__) && !defined(__UWIN__)
#define inhibit_libc
#endif
...
{
  char *buf = alloca (sizeof ("4294967295") + strlen (string) + strlen (expression) + strlen (filename) + 2); 
  sprintf (buf, string, expression, line, filename);
  ...
  if (!strcmp (bb_head->filename+i, ".da"))
      ...
      for (ptr = bb_head; ptr != (struct bb *) 0; ptr = ptr->next)
	{
	  ...
	  if ((da_file = fopen (ptr->filename, "r")) != 0)
		...
		{
#		  define MSG "arc profiling: Can't read output file %s.\n"
		  char buf[sizeof(MSG) + MAXPATHLEN];
		  sprintf (buf, MSG, ptr->filename);
		  write (2, buf, strlen (buf));
#		  undef MSG
		  continue;
		}
...
__eh_rtime_match (void *rtime)
{
  ...
  if (! matcher)
    {
#ifndef inhibit_libc
      write (2, "Internal Compiler Bug: No runtime type matcher.\n",
	     sizeof ("Internal Compiler Bug: No runtime type matcher.\n") - 1);
#endif
      return 0;
    }
